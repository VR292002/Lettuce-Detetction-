...
async def display_table():
    try:
        ...
        # Include both 'image' and 'photos' columns in the DataFrame
        df = df[['image'] + [col for col in df.columns if col != 'image'] + ['photos']]
        ...
        for index, row in df.iterrows():
            for category in ['type', 'disease', 'pest']:
                for item, confidence in row['predictions'][category]:
                    new_rows.append({'image': row['image'], 'photos': row['photos'], 'category': category, 'item': item, 'confidence': confidence, 'timestamp': row['timestamp']})
