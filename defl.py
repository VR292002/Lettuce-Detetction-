        ...
        for category in ['type', 'disease', 'pest']:
            for item in doc['predictions'][category]:
                record = {
                    ...
                    'Photos': f"<img src='{doc['image']}' width='auto' height='auto'>",
                    'category': category,
                    'item_name': item[0],
                    'confidence': item[1],
...
def home():
    if request.method == 'POST':
        ...
        return render_template('index.html', tables=[filtered_df.to_html(classes='data', escape=False, index=False)], titles=filtered_df.columns.values)
